CREATE PROCEDURE SessionsResultsPostDeps2
AS
...
WHILE @StudentsNum <= @QuantityStudents
	BEGIN
		...
		(SELECT Students.GroupsID FROM Students Where Students.ID = @StudentsNum) AND ExamSchedules.SessionsID = 1)
		...
		BEGIN
			...
			AND ExamSchedules.SubjectsID = @QuantitySubjects AND ExamSchedules.SessionsID = 1);
